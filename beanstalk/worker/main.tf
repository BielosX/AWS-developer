...
resource "aws_elastic_beanstalk_application" "worker-app" {
...
resource "aws_elastic_beanstalk_application_version" "init-version" {
  ...
  application = aws_elastic_beanstalk_application.worker-app.id
  bucket = aws_s3_bucket.deployment-bucket.id
...
resource "aws_elastic_beanstalk_environment" "worker-env" {
  application = aws_elastic_beanstalk_application.worker-app.name
  name = "worker-env"
  tier = "Worker"
